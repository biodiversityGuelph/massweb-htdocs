...
if($searchTerms[0] <> "")
{
	...
	{
		$searchTerms[$i] = 'process_id = "' . $searchTerms[$i] . '"';
...
{
	$error = "Error retrieving results for query: <br>" . $sql . "<br>" . $e->getMessage();
...
foreach($s as $row)
{
	$results[] = array('id' => $row['id'], 'process_id' => $row['process_id'], 'phylum' => $row['phylum'], 
