...
if($searchTerms[0] <> "")
{
	...
	{
		$searchTerms[$i] = 'project_id = (SELECT id FROM db189691_massbase.project WHERE lower(name) = lower("' . $searchTerms[$i] . '")) 
							OR project_id = (SELECT id FROM db189691_massbase.project WHERE lower(project_code)  
							= lower("' . $searchTerms[$i] . '")) ';
...
{
	$error = "Error retrieving project results. <br>" . $e->getMessage();
